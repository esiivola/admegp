...
real log_diff_exp_abs(real la, real lb) {
  return(0.5 * log_diff_exp(log_sum_exp(2.0*la, 2.0*lb), log(2.0) + la + lb));
}
...
void check_order_increasing(vector test) {
  for(i in 1:(rows(test)-1))
    if(test[i] > test[i+1])
      reject("Test is not increasing monotonically.");
}

void check_order_increasing_blocked(int[] M, vector test) {
  int B = num_elements(M);
  int b = 1;
  for(m in 1:B) {
    check_order_increasing(test[b:M[m]]);
    b += M[m];
  }  
}


// returns a vector which is as long as left_id which corresponds
// to a left join with the values in right which are mapped to left_id
// by right_id
vector left_join_vector(int[] left_id, vector right, int[] right_id) {
   int L = size(left_id);
   int R = size(right_id);
   vector[L] left;
   if( cardinality_int(right_id) != R )
     reject("right id set must be unique!");
   if( num_elements(right) != R )
     reject("right_id must match in size to right.");
   ...
   for(l in 1:L) {
      ...
      if(!found)
        reject("Did not find element ", lid, " in right id set.");
  }
  return(left);
}

int[] left_join_int(int[] left_id, int[] right, int[] right_id) {
   int L = size(left_id);
   int R = size(right_id);
   int left[L];
   if( cardinality_int(right_id) != R )
...
// return the adjacent difference
int[] adjacent_difference_int(int[] elems) {
  int num_out = size(elems)-1;
  int diff[num_out];
  for(i in 1:num_out) {
    diff[i] = elems[i+1] - elems[i];
  }
  return diff;
}
